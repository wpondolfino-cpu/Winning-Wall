...
import { Workout, Score, upsertScore, getVideoId, updateStreak, STREAK_BONUS_DAYS, STREAK_BONUS_PTS } from "../lib/supabase";
...
export default function WorkoutsPanel({ workouts, myScores, playerId, onScoreLogged }: Props) {
  ...
  async function submitScore() {
    ...
    try {
      ...
      const msg = bonusAwarded
        ? `🔥 ${STREAK_BONUS_DAYS}-day streak! You earned ${STREAK_BONUS_PTS} bonus points!`
        : newStreak >= 2
        ? `Score logged! 🔥 ${newStreak}-day streak!`
        : "Score logged! Keep grinding 💪";
      showToast(msg);
      ...
      {activeWorkout && (
        <div className="modal-overlay open" onClick={() => setActiveWorkout(null)}>
          <div className="log-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-title">{scoreFor(activeWorkout.id) ? "Update Score" : "Log Score"}</div>
    <div style={{ fontSize: 13, color: "var(--muted)", marginBottom: 16 }}>{activeWorkout.title}</div>
    {scoreFor(activeWorkout.id) && (
      <div style={{ padding: "8px 12px", background: "rgba(26,63,168,0.12)", borderRadius: 8, fontSize: 12, color: "#93b4ff", marginBottom: 14 }}>
        You already have a score for this workout. Submitting will update it.
      </div>
    )}
