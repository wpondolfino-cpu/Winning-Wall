...
export async function submitScore(
  ...
  // 5. Re-rank ALL players for this workout and update their points
  // Get all scores for this workout
  const { data: allWorkoutScores } = await supabase
    .from("scores")
    .select("id, player_id, made, reps, sprint_secs, self_points")
    .eq("workout_id", score.workout_id);

  if (allWorkoutScores && allWorkoutScores.length > 0) {
    // Sort by raw score descending to determine rank
    const sorted = [...allWorkoutScores]
      .map(s => ({ ...s, raw: computeRawScore(s) }))
      .sort((a, b) => b.raw - a.raw);

    // Assign points based on rank and update each player's score row
    for (let i = 0; i < sorted.length; i++) {
      const rank = i + 1;
      const pts = rank === 1 ? firstPts : rank === 2 ? secondPts : rank === 3 ? thirdPts : 0;
      await supabase
        .from("scores")
        .update({ points: pts })
        .eq("id", sorted[i].id);
