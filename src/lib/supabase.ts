...
export async function submitScore(
  ...
  // 1. Check if a personal best already exists for this workout
  ...
  // 2. Always log this attempt in score_attempts (for streak + history)
  // Fetch the workout to get custom rank points
  const { data: workout } = await supabase
    .from("workouts")
    .select("first_place_pts, second_place_pts, third_place_pts, scoring_type, grade_category")
  ...
  // 3. Only update the leaderboard score if this is a new personal best
  let saved: Score;
  if (isPersonalBest) {
    const { data, error } = await supabase
      .from("scores")
    ...
    // Score not a PB — just return existing best unchanged
