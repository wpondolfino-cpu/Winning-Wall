...
export default function Leaderboard({ currentUserId }: Props) {
  ...
  function buildPeriodBoard() {
    // Sum points earned per player within this period using score_attempts
    ...
    for (const s of periodScores) {
      ...
      // Only count personal best attempts toward period points
      if ((s as any).is_personal_best) {
        map[s.player_id].period_points += (s as any).raw_score ?? 0;
      }
      map[s.player_id].workouts_logged += 1;
    }
  ...
  // ── Shared row renderer ──
  function renderWorkoutTable(board: ReturnType<typeof getWorkoutBoard>) {
    return (
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        {board.length === 0 && (
          <div style={{ padding: 28, textAlign: "center", color: "var(--muted)", fontSize: 13 }}>No scores yet. Be the first! 🏀</div>
        )}
      </div>
    );
  }
