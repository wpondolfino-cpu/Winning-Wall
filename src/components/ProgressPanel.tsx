...
import { BADGES, checkNewBadges, PlayerStats } from "../lib/badges";
...
export default function ProgressPanel({ profile, myScores, workouts }: Props) {
  ...
  const tabBtn = (t: "bests" | "history", label: string) => (
  ...
  return (
    <div className="panel active">
      ...
      {view === "badges" && (() => {
        ...
        const notEarned = BADGES.filter(b => !b.check(stats));
        return (
          <div className="card">
            <div className="card-title">Your Badges — {earned.length}/{BADGES.length} earned</div>
            {earned.length > 0 && (
