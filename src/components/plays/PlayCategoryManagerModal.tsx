// src/components/plays/PlayCategoryManagerModal.tsx
// Add/rename/delete play categories. Renaming cascades to every play
// using that category (handled by the DB's ON UPDATE CASCADE);
// deleting is blocked while any play still uses it. Mirrors
// CategoryManagerModal.tsx (drill categories) exactly.

import { useState, useEffect } from "react";
import { PlayCategory, getPlayCategories, addPlayCategory, renamePlayCategory, deletePlayCategory } from "../../lib/playCategories";

interface Props {
  onClose: () => void;
  onChanged: () => void; // let the parent (PlayViewer) refresh its own category list
}

export default function PlayCategoryManagerModal({ onClose, onChanged }: Props) {
  ...
  async function load() {
    setLoading(true);
    setCategories(await getPlayCategories());
    setLoading(false);
  }
  ...
  async function handleAdd() {
    ...
    setAdding(true);
    setError(null);
    const { error } = await addPlayCategory(newName);
  ...
  async function handleRename(oldName: string) {
    setBusyName(oldName);
    setError(null);
    const { error } = await renamePlayCategory(oldName, editValue);
    if (error) { setError(error); }
  ...
  async function handleDelete(name: string) {
    if (!window.confirm(`Delete "${name}"? This only works if no plays currently use it.`)) return;
    setBusyName(name);
    setError(null);
    const { error } = await deletePlayCategory(name);
    if (error) { setError(error); }
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, width: "min(420px, 96vw)", padding: 22 }}>
        ...
        <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 16 }}>
          Used to organize your plays. Renaming updates every play that uses it.
        </div>
