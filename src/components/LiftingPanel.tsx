...
function getBestSet(setsData: any[]): any {
  if (!setsData || setsData.length === 0) return null;
  return setsData.reduce((max: any, s: any) =>
    calc1RM(s.weight, s.reps) > calc1RM(max?.weight ?? 0, max?.reps ?? 0) ? s : max, null);
}
...
export default function LiftingPanel({ playerId, playerName, avatarUrl, isCoach = false, isAdmin = false }: Props) {
  ...
  const [firstLogs, setFirstLogs] = useState<Record<string, any>>({});

  // ── coach results modal ──
  const [resultsProgram, setResultsProgram] = useState<Program | null>(null);
  const [resultsData, setResultsData] = useState<any[]>([]); // per player per exercise
  const [resultsLoading, setResultsLoading] = useState(false);

  // ── builder state ──
  const [bTitle, setBTitle] = useState("");
  const [bDesc, setBDesc] = useState("");
  const [bVisibility, setBVisibility] = useState<"public" | "assigned" | "personal">("public");
  const [bIsPersonal, setBIsPersonal] = useState(false);
  ...
  async function loadPrograms() {
    ...
    try {
      const { data: progs } = await supabase
        .from("lifting_programs").select("*").eq("is_active", true)
        .order("created_at", { ascending: false });
      if (!progs) { setLoading(false); return; }

      let visible: Program[] = [];
      if (canManage) {
        // Coaches see everything except other players' personal programs
        visible = progs.filter(p => p.visibility !== "personal");
      } else {
        const { data: assignments } = await supabase
          .from("lifting_program_assignments").select("program_id").eq("player_id", playerId);
        const assignedIds = new Set((assignments ?? []).map((a: any) => a.program_id));
        visible = progs.filter(p =>
          p.visibility === "public" ||
      ...
      if (visible.length > 0) {
        const { data: exs } = await supabase
          .from("lifting_exercises").select("*")
          .in("program_id", visible.map(p => p.id)).order("sort_order");
  ...
  // Load last 3 logs per exercise for player view
  async function loadRecentLogs(exerciseIds: string[]) {
    if (exerciseIds.length === 0) return;
    const { data } = await supabase
      .from("lifting_logs").select("*").eq("player_id", playerId)
      .in("exercise_id", exerciseIds).order("logged_at", { ascending: false });
    const byEx: Record<string, any[]> = {};
    const firstByEx: Record<string, any> = {};
    (data ?? []).forEach((log: any) => {
      if (!byEx[log.exercise_id]) byEx[log.exercise_id] = [];
      if (byEx[log.exercise_id].length < 3) byEx[log.exercise_id].push(log);
    });
    // Get first log per exercise for growth calculation
    const asc = [...(data ?? [])].reverse();
    asc.forEach((log: any) => { firstByEx[log.exercise_id] = log; });
    setRecentLogs(prev => ({ ...prev, ...byEx }));
    setFirstLogs(prev => ({ ...prev, ...firstByEx }));
  }

  // Load ALL player logs for a program (coach results view)
  async function loadResults(prog: Program) {
    setResultsProgram(prog);
    setResultsLoading(true);
    setResultsData([]);
    try {
      const exs = exercises[prog.id] ?? [];
      if (exs.length === 0) { setResultsLoading(false); return; }

      // Get all logs for these exercises
      const { data: logs } = await supabase
        .from("lifting_logs").select("*")
        .in("exercise_id", exs.map(e => e.id))
        .order("logged_at", { ascending: true });

      // Get all player profiles who have logged
      const playerIds = [...new Set((logs ?? []).map((l: any) => l.player_id))];
      if (playerIds.length === 0) { setResultsLoading(false); return; }

      const { data: profiles } = await supabase
        .from("profiles").select("id,name,avatar_url").in("id", playerIds);

      // Build results: per player, per exercise — first log, last log, best set
      const byPlayerEx: Record<string, Record<string, { first: any; last: any; allLogs: any[] }>> = {};
      (logs ?? []).forEach((log: any) => {
        if (!byPlayerEx[log.player_id]) byPlayerEx[log.player_id] = {};
        if (!byPlayerEx[log.player_id][log.exercise_id]) {
          byPlayerEx[log.player_id][log.exercise_id] = { first: log, last: log, allLogs: [] };
        }
        byPlayerEx[log.player_id][log.exercise_id].last = log;
        byPlayerEx[log.player_id][log.exercise_id].allLogs.push(log);
      });

      const rows = (profiles ?? []).map((p: any) => ({
        player: p,
        exercises: exs.map(ex => {
          const data = byPlayerEx[p.id]?.[ex.id];
          if (!data) return { exercise: ex, hasData: false };
          const firstBest = getBestSet(data.first.sets_data);
          const lastBest = getBestSet(data.last.sets_data);
          const firstWeight = firstBest?.weight ?? 0;
          const lastWeight = lastBest?.weight ?? 0;
          const growth = lastWeight - firstWeight;
          const sessions = data.allLogs.length;
          return { exercise: ex, hasData: true, firstWeight, lastWeight, growth, sessions, lastLog: data.last };
        }),
      }));
  ...
  function openLog(ex: Exercise) { setLogExercise(ex); setSets([{ reps: "", weight: "" }]); }
  ...
  async function saveLog() {
    ...
    try {
      ...
      await supabase.from("lifting_logs").insert({
        player_id: playerId, exercise_id: logExercise.id,
        logged_at: new Date().toISOString(), sets_data: setsData,
      });
      if (logExercise.hof_eligible) {
        ...
        const { data: existing } = await supabase.from("lifting_records").select("*")
          .eq("player_id", playerId).eq("exercise_id", logExercise.id).single();
        if (!existing || best.rm > existing.best_1rm) {
          await supabase.from("lifting_records").upsert({
            player_id: playerId, exercise_id: logExercise.id,
            player_name: playerName, avatar_url: avatarUrl ?? null,
            best_weight: best.weight, best_1rm: best.rm,
    ...
    } catch (e: any) { showLogToast("Error: " + e.message); }
    finally { setLogSaving(false); }
  }

  function showLogToast(msg: string) { setLogToast(msg); setTimeout(() => setLogToast(""), 3000); }

  function openBuilder(prog?: Program, forcePersonal = false) {
    setBIsPersonal(forcePersonal);
    if (prog) {
      setEditProgram(prog);
      setBTitle(prog.title); setBDesc(prog.description ?? "");
      ...
      setEditProgram(null); setBTitle(""); setBDesc("");
      setBVisibility(forcePersonal ? "personal" : "public");
      setBExercises([{ name: "", video_url: "", target_sets: 3, target_reps: 8, target_weight: undefined, sort_order: 0, hof_eligible: false }]);
      setAssignedPlayers([]);
    }
    if (!forcePersonal) loadAllPlayers();
    setView("builder");
  ...
  async function saveProgram() {
    ...
    try {
      const finalVisibility = bIsPersonal ? "personal" : bVisibility;
      let programId: string;
      if (editProgram) {
        await supabase.from("lifting_programs").update({ title: bTitle, description: bDesc || null, visibility: finalVisibility }).eq("id", editProgram.id);
        ...
        const { data, error } = await supabase.from("lifting_programs").insert({
          created_by: playerId, title: bTitle, description: bDesc || null,
          visibility: finalVisibility, is_active: true,
        }).select().single();
        if (error) throw error;
        programId = data.id;
      }
      const validExercises = bExercises.filter(e => e.name.trim()).map((e, i) => ({
        program_id: programId, name: e.name.trim(),
        video_url: e.video_url?.trim() || null, target_sets: e.target_sets || null,
        target_reps: e.target_reps || null, target_weight: e.target_weight || null,
        sort_order: i, hof_eligible: e.hof_eligible,
      }));
      await supabase.from("lifting_exercises").insert(validExercises);
      if (finalVisibility === "assigned" && assignedPlayers.length > 0) {
        await supabase.from("lifting_program_assignments").insert(
          assignedPlayers.map(pid => ({ program_id: programId, player_id: pid }))
        );
      }
      setView("programs"); await loadPrograms();
    ...
    return (
      <div className="panel active">
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <div className="section-title" style={{ margin: 0 }}>{editProgram ? "Edit Program" : bIsPersonal ? "My Personal Program" : "New Lifting Program"}</div>
        </div>
        <div className="builder-form">
          <div>
            <label>Program Title</label>
            <input value={bTitle} onChange={e => setBTitle(e.target.value)} placeholder={bIsPersonal ? "e.g. My Summer Lifts" : "e.g. Summer Strength Phase 1"} />
          </div>
          <div>
            <label>Description (optional)</label>
            <textarea value={bDesc} onChange={e => setBDesc(e.target.value)} placeholder="Any notes about this program…" rows={2} />
          </div>

          {bIsPersonal ? (
            <div style={{ padding: "10px 14px", background: "rgba(26,63,168,0.08)", border: "1px solid rgba(26,63,168,0.2)", borderRadius: 8, fontSize: 12, color: "var(--silver-light)", lineHeight: 1.6 }}>
              🔒 This program is private — only you can see it.
            </div>
          ) : canManage && (
            <div>
          ...
          {!bIsPersonal && bVisibility === "assigned" && canManage && (
            <div>
              ...
              {bExercises.map((ex, i) => (
                <div key={i} style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 12, padding: 14 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 8 }}>
                    {["Sets", "Reps", "Weight (lbs)"].map((lbl, fi) => (
                      <div key={lbl}>
                        <div style={{ fontSize: 10, color: "var(--muted)", marginBottom: 3, textTransform: "uppercase", letterSpacing: 0.4 }}>{lbl}</div>
                        <input type="number"
                          value={fi === 0 ? ex.target_sets ?? "" : fi === 1 ? ex.target_reps ?? "" : ex.target_weight ?? ""}
                          onChange={e => updateExercise(i, fi === 0 ? "target_sets" : fi === 1 ? "target_reps" : "target_weight", fi === 2 ? parseFloat(e.target.value) || undefined : parseInt(e.target.value) || undefined)}
                          placeholder={fi === 0 ? "3" : fi === 1 ? "8" : "135"}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button className="btn-primary" onClick={saveProgram} disabled={bSaving} style={{ flex: 1 }}>
              {bSaving ? "Saving…" : editProgram ? "Save Changes" : bIsPersonal ? "Save My Program" : "Publish Program"}
            </button>
            <button onClick={() => setView("programs")} style={{ background: "var(--surface2)", color: "var(--muted)", border: "1px solid var(--border)", borderRadius: 10, padding: "12px 20px", fontFamily: "inherit", fontSize: 14, cursor: "pointer" }}>Cancel</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="panel active">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {!canManage && (
            <button onClick={() => openBuilder(undefined, true)} style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--silver-light)", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 600, fontFamily: "inherit", cursor: "pointer" }}>
              + My Program
            </button>
          )}
          {canManage && <button onClick={() => openBuilder()} className="coach-add-btn">+ New Program</button>}
        </div>
      </div>
      ...
      {coachPrograms.length === 0 && personalPrograms.length === 0 ? (
        ...
        <>
          {coachPrograms.length > 0 && (
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {coachPrograms.map(prog => renderProgram(prog))}
              </div>
            </div>
          )}
          {personalPrograms.length > 0 && (
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {personalPrograms.map(prog => renderProgram(prog))}
              </div>
            </div>
          )}
        </>
      ...
      {logExercise && (
        <div className="modal-overlay open" onClick={() => setLogExercise(null)}>
          <div className="log-modal" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={addSet} style={{ width: "100%", background: "none", border: "1px dashed var(--border)", borderRadius: 8, padding: "9px", fontSize: 13, color: "var(--muted)", cursor: "pointer", fontFamily: "inherit", marginBottom: 16 }}>+ Add Set</button>
            {(() => {
              const validSets = sets.filter(s => s.reps.trim() && s.weight.trim());
              if (validSets.length === 0) return null;
              const best = validSets.reduce((max, s) => { const rm = calc1RM(parseFloat(s.weight), parseInt(s.reps)); return rm > max ? rm : max; }, 0);
              return best > 0 ? (
            ...
            <button className="btn-primary" onClick={saveLog} disabled={logSaving}>{logSaving ? "Saving…" : "Save Session"}</button>
          </div>
        </div>
      )}
      ...
      {/* Coach Results Modal */}
      {resultsProgram && (
        <div className="modal-overlay open" onClick={() => setResultsProgram(null)}>
          <div className="log-modal" style={{ maxWidth: 640, width: "95vw" }} onClick={e => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setResultsProgram(null)}>✕</button>
            <div className="modal-title" style={{ marginBottom: 4 }}>📊 Player Results</div>
            <div style={{ fontSize: 13, color: "var(--muted)", marginBottom: 16 }}>{resultsProgram.title}</div>
                  ...
                  return (
                    <div key={player.id} style={{ background: "var(--surface2)", borderRadius: 12, border: "1px solid var(--border)", overflow: "hidden" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderBottom: "1px solid var(--border)", background: "rgba(0,0,0,0.1)" }}>
                        <div style={{ width: 32, height: 32, borderRadius: "50%", overflow: "hidden", border: "2px solid var(--border)", background: "rgba(26,63,168,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          {player.avatar_url
                      ...
                      <div style={{ overflowX: "auto" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 80px 80px 70px", gap: 6, padding: "8px 14px", fontSize: 9, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.5, minWidth: 400 }}>
                          <div>Exercise</div>
                        ...
                        {exResults.map((r: any) => {
                          ...
                          return (
                            <div key={r.exercise.id} style={{ display: "grid", gridTemplateColumns: "1fr 80px 80px 80px 70px", gap: 6, padding: "8px 14px", borderTop: "1px solid var(--border)", alignItems: "center", minWidth: 400 }}>
                              <div style={{ fontSize: 12, color: "var(--text)", fontWeight: 600 }}>{r.exercise.name}</div>
  ...
  function renderProgram(prog: Program) {
    ...
    const isPersonalProg = prog.visibility === "personal";
    const canEditThis = canManage || (isPersonalProg && prog.created_by === playerId);

    return (
      <div key={prog.id} style={{ background: "var(--surface2)", border: `1px solid ${isPersonalProg ? "rgba(147,180,255,0.3)" : "var(--border)"}`, borderRadius: 14, overflow: "hidden" }}>
        <div onClick={() => {
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text)", display: "flex", alignItems: "center", gap: 8 }}>
              {prog.title}
              {isPersonalProg && <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 6px", borderRadius: 5, background: "rgba(147,180,255,0.15)", color: "#93b4ff" }}>PERSONAL</span>}
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            {canManage && (
              <button onClick={e => { e.stopPropagation(); loadResults(prog); }}
                style={{ background: "rgba(26,63,168,0.2)", border: "1px solid rgba(26,63,168,0.4)", color: "#93b4ff", borderRadius: 7, padding: "5px 10px", fontSize: 11, fontWeight: 600, fontFamily: "inherit", cursor: "pointer" }}>
                📊 Results
              </button>
            )}
            {canEditThis && (
              <>
                <button onClick={e => { e.stopPropagation(); openBuilder(prog, isPersonalProg); }}
        ...
        {isExpanded && (
          <div style={{ borderTop: "1px solid var(--border)" }}>
            {exs.length === 0 ? (
              ...
              <div>
                {/* Spreadsheet header — 6 columns now with Progress */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 50px 50px 70px 80px 70px", gap: 6, padding: "10px 18px", background: "rgba(0,0,0,0.15)", fontSize: 9, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.5 }}>
                  <div>Exercise</div>
                ...
                {exs.map(ex => {
                  ...
                  const firstLog = firstLogs[ex.id];
                  const lastLog = logs[0];
                  const lastSets = lastLog?.sets_data ?? [];
                  const firstSets = firstLog?.sets_data ?? [];
                  const bestLastSet = getBestSet(lastSets);
                  const bestFirstSet = getBestSet(firstSets);
                  const lastWeight = bestLastSet?.weight ?? 0;
                  const firstWeight = bestFirstSet?.weight ?? 0;
                  const growth = lastWeight > 0 && firstWeight > 0 ? lastWeight - firstWeight : null;
                  const vid = getYouTubeId(ex.video_url);
                  return (
                    <div key={ex.id} style={{ borderTop: "1px solid var(--border)" }}>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 50px 50px 70px 80px 70px", gap: 6, padding: "12px 18px", alignItems: "center" }}>
                        <div>
                          ...
                          {vid && (
                            <a href={`https://youtube.com/watch?v=${vid}`} target="_blank" rel="noreferrer"
                              style={{ fontSize: 10, color: "var(--gold)", textDecoration: "none", fontWeight: 600, marginTop: 4, display: "inline-block" }}>📹 Demo ↗</a>
                          )}
                        </div>
                        <div style={{ textAlign: "center", fontSize: 12, color: "var(--silver-light)", fontWeight: 600 }}>{ex.target_sets ?? "—"}</div>
                        <div style={{ textAlign: "center", fontSize: 12, color: "var(--silver-light)", fontWeight: 600 }}>{ex.target_reps ?? "—"}</div>
                        <div style={{ textAlign: "center", fontSize: 12, color: "var(--silver-light)", fontWeight: 600 }}>{ex.target_weight ? `${ex.target_weight} lbs` : "—"}</div>
                        <div style={{ textAlign: "center" }}>
                          {bestLastSet ? (
                            <div>
                              <div style={{ fontSize: 12, fontWeight: 700, color: "#5de098" }}>{bestLastSet.weight} lbs</div>
                              <div style={{ fontSize: 10, color: "var(--muted)" }}>×{bestLastSet.reps} reps</div>
                            </div>
