...
export default function Leaderboard({ currentUserId }: Props) {
  ...
  useEffect(() => { loadData(); loadHof(); loadRecords(); }, []);
            ...
            {(() => {
              ...
              return (
                <div>
                  ...
                  {allRecords.length === 0 ? (
                    <div style={{ fontSize: 13, color: "var(--muted)", padding: "20px 0" }}>No records yet — start logging workouts to set them!</div>
                  ) : (
